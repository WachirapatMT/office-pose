...
class Application(tk.Frame):
  def __init__(self, master=None):
    ...
    self.render_keypoint_sets = None
  ...
  def update(self):
    ...
    if self.keypoint_sets is not None and len(self.keypoint_sets) > 0:
      if self.render_keypoint_sets is None:
        self.render_keypoint_sets = self.keypoint_sets[0][:]
      else:
        for i, (x, y, _) in enumerate(self.render_keypoint_sets):
          self.render_keypoint_sets[i][0] = self.render_keypoint_sets[i][0] * INTERPOLATION_PROPORTION \
            + self.keypoint_sets[0][i][0] * (1 - INTERPOLATION_PROPORTION)
          self.render_keypoint_sets[i][1] = self.render_keypoint_sets[i][1] * INTERPOLATION_PROPORTION \
            + self.keypoint_sets[0][i][1] * (1 - INTERPOLATION_PROPORTION)
      ...
      for i, (p1_index, p2_index, _) in enumerate(SKELETON_CONNECTIONS):
        p1 = (self.render_keypoint_sets[p1_index][0] * WIDTH, self.render_keypoint_sets[p1_index][1] * HEIGHT)
        p2 = (self.render_keypoint_sets[p2_index][0] * WIDTH, self.render_keypoint_sets[p2_index][1] * HEIGHT)
      ...
      if self.keypoint_ovals is None:
        self.keypoint_ovals = [None]*len(self.render_keypoint_sets)
      for i, keypoint in enumerate(self.render_keypoint_sets):
